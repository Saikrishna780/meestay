...
import { LayoutDashboard, UserCheck, IndianRupee, Zap, Plus, Save, AlertCircle, ArrowRightLeft, BarChart2 } from 'lucide-react'
    ...
    <div style={{ padding: 16, background: '#F9FAFB', borderRadius: 10, border: '1px solid #E5E7EB', marginBottom: 16 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12, marginBottom: 12 }}>
        <div>
          ...
          <select value={form.meterType} onChange={e => handleMeterTypeChange(e.target.value)}
            ...
            <option value="MAIN">Main Meter (All Tenants)</option>
            <option value="SUB">Sub Meter (AC Room Only)</option>
          </select>
        </div>
          ...
          <div>
            ...
            {acRooms.length === 0 ? (
              <div style={{ marginTop: 4, padding: '8px 10px', borderRadius: 8, background: '#FEF3C7', border: '1px solid #FDE68A', fontSize: 12, color: '#92400E' }}>
                No AC rooms found in this hostel
              </div>
            ) : (
      ...
      {form.meterType === 'SUB' && form.roomId && (
        <div style={{ padding: '10px 14px', borderRadius: 8, background: '#EEF2FF', border: '1px solid #C7D2FE', marginBottom: 12, fontSize: 13, color: '#4338CA' }}>
          ⚡ {form.meterLabel} — Only AC room tenants will be charged for electricity
        </div>
      )}
      ...
      {form.meterType === 'MAIN' && (
        <div style={{ padding: '10px 14px', borderRadius: 8, background: '#F0FDF4', border: '1px solid #A7F3D0', marginBottom: 12, fontSize: 13, color: '#065F46' }}>
          ℹ️ Main meter cost will be split equally among all AC room tenants only. Non-AC tenants are excluded.
        </div>
      )}
...
export default function ManagerPower() {
  ...
  const load = () => {
    const url = filterMonth ? `/manager/power/readings?month=${filterMonth}` : '/manager/power/readings'
    api.get(url).then(r => setReadings(r.data.data || [])).catch(() => {})
  }

  useEffect(() => {
    load()
    api.get('/manager/power/ac-rooms').then(r => setAcRooms(r.data.data || [])).catch(() => {})
  }, [filterMonth])
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: '#6B7280', marginTop: 2 }}>
              AC rooms only — Non-AC tenants are excluded from power billing
            </p>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 16 }}>
          {[
            ...
            { label: 'This Month', value: readings.filter(r => r.readingMonth === currentMonth()).length, bg: '#FEF3C7', color: '#92400E' },
        ...
        {showForm && (
          <Card title="Add Meter Reading">
            <ReadingForm onSave={() => { load(); setShowForm(false) }} acRooms={acRooms} apiPrefix="manager" />
          </Card>
        )}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#F9FAFB', borderBottom: '2px solid #E5E7EB' }}>
                  {['Type', 'Room / Label', 'Month', 'Prev', 'Current', 'Consumed', '₹/Unit', 'Total'].map(h => (
                  ...
                  : readings.map(r => {
                    ...
                    return (
                      <tr key={r.id} style={{ borderBottom: '1px solid #F3F4F6' }}>
                        ...
                        <td style={{ padding: '10px 14px', fontSize: 13, fontWeight: 700, color: '#059669' }}>₹{cost.toFixed(2)}</td>
                      </tr>
                    )
                  })}
              </tbody>
